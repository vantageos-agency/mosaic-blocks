/**
 * saveChatAsMemoryFormVariants — pure CVA variant functions for
...
export const saveChatAsMemoryFormRootVariants = cva(["flex flex-col gap-4"]);

/** Field group (label + control + helper/error) vertical layout. */
export const saveChatAsMemoryFormFieldVariants = cva(["flex flex-col gap-1.5"]);

/** Field error text. */
export const saveChatAsMemoryFormErrorVariants = cva(["text-xs font-medium text-destructive"]);

/** Footer action row. */
export const saveChatAsMemoryFormFooterVariants = cva([
  "flex items-center justify-end gap-3 border-t border-border pt-4",
]);

/** Save / cancel button variants by intent. */
export const saveChatAsMemoryFormButtonVariants = cva(
  [

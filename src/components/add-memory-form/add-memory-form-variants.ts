/**
 * addMemoryFormVariants — pure CVA variant functions for MosaicAddMemoryForm.
 *
...
export const addMemoryFormRootVariants = cva(["flex flex-col gap-4"]);

/** Field group (label + control + helper/error) vertical layout. */
export const addMemoryFormFieldVariants = cva(["flex flex-col gap-1.5"]);

/** Field error text. */
export const addMemoryFormErrorVariants = cva(["text-xs font-medium text-destructive"]);

/** Footer action row. */
export const addMemoryFormFooterVariants = cva([
  "flex items-center justify-end gap-3 border-t border-border pt-4",
]);

/** Submit / cancel button variants by intent. */
export const addMemoryFormButtonVariants = cva(
  [
  ...
  ],
  {
    variants: {
      intent: {
        ...
        submit: "border border-transparent bg-primary text-primary-foreground hover:bg-primary/90",
      },
    },
    defaultVariants: {
      intent: "cancel",

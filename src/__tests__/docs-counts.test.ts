...
function baseCatalog(mosaicCount: number, totalExports: number): string {
  return [
    ...
    "",
    `It documents **1 \`Mosaic*\` components** and **0 hooks** <!-- count-exempt: curated-subset figures, not derived from src/index.ts --> out of the **${mosaicCount}** \`Mosaic*\``,
...
describe("docs-counts.mjs — per-anchor blind spot: losing ONE anchor out of ten must never be hidden by the other nine", () => {
  ...
  function fullCatalog(n: number): string {
    return [
      ...
      "## Documented / exported ratio",
      "",
      `It documents **1 \`Mosaic*\` components** and **0 hooks** <!-- count-exempt: curated-subset figures, not derived from src/index.ts --> out of the **${n}** \`Mosaic*\``,
